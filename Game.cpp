...
void Game::Tick(InputCommands *Input, RECT* clientRectInput)
{
	...
	//pass in the DX Window
	clientRect = *clientRectInput;
...
void Game::Update(DX::StepTimer const& timer)
{
	...
	int deltaMouseX = oldMouseX - m_InputCommands.mouseX;
	oldMouseX = m_InputCommands.mouseX;
	...
	{
		m_camOrientation.y += deltaMouseX;
		m_camOrientation.z += deltaMouseY;

	}

	//create look direction from Euler angles in m_camOrientation
	m_camLookDirection.x = sin((m_camOrientation.y)*3.1415 / 180) * sin((m_camOrientation.z)*3.1415 / 180);
	m_camLookDirection.z = cos((m_camOrientation.y)*3.1415 / 180) * sin((m_camOrientation.z)*3.1415 / 180);
	m_camLookDirection.y = cos((m_camOrientation.z)*3.1415 / 180);
	...
	//update any selected objects
	if (currentSelection != oldSelection)
	{
		if (currentSelection != -1)
		{
			m_displayList[currentSelection].m_scale *= 1.5f;
		}

		if (oldSelection != -1)
		{
			m_displayList[oldSelection].m_scale /= 1.5f;
		}
	}
...
void Game::PickObject()
{
	...
	for (int i = 0; i < m_displayList.size(); i++)
	{
		...
		{
			float distance;
			if (m_displayList[i].m_model.get()->meshes[y]->boundingBox.Intersects(nearPoint, direction, distance))
			{
	...
	if (pickedObjectID > -1)
	{
		currentSelection = pickedObjectID;
	}
}
...
void Game::SetManipulationState(manipulationState state)
{
