...
/*
void ToolMain::onActionFocusX()
{
	m_d3dRenderer.SetManipulationState(X);
}

void ToolMain::onActionFocusY()
{
	m_d3dRenderer.SetManipulationState(Y);
}

void ToolMain::onActionFocusZ()
{
	m_d3dRenderer.SetManipulationState(Z);
}

void ToolMain::onActionClear()
{
	m_d3dRenderer.SetManipulationState(None);
}

void ToolMain::onActionScale()
{
	m_d3dRenderer.SetManipulationState(Scale);
}
...
void ToolMain::Tick(MSG *msg)
{
	...
	m_d3dRenderer.Tick(&m_toolInputCommands, &clientRect);

	m_selectedObject = m_d3dRenderer.ReturnPickedSelection();
}
...
void ToolMain::UpdateInput(MSG * msg)
{
	...
	case WM_LBUTTONDOWN:
		lMouseDown = true;	//mouse button down,  you will probably need to check when its up too
	...
	else m_toolInputCommands.rightClick = false;

	if (lMouseDown)
	{
		m_toolInputCommands.leftClick = true;
	}
	else m_toolInputCommands.leftClick = false;

	m_toolInputCommands.mouseX = mouseX;
	m_toolInputCommands.mouseY = mouseY;
